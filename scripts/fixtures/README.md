...
These 39 files (22 CSVs, 3 CSV + 3 JSON pairs added for R-H — see
"Forced-alignment fixture" below — plus 5 JSON vocab fixtures, see
"Forced-alignment shipping-model vocabularies" — plus 6 CSVs added 2026-08-16
for R-H's second baseline, see its own section below) are **inputs read by
hardcoded path**, not documentation: by `scripts/phase4-handoff-replay-sync.test.ts` (the
...
`phase4-fa-tokens-*.json`/`phase4-fa-baseline-*-words.csv` pair, regenerated
2026-08-16 (R-Q) — see "Forced-alignment fixture" below for why. Nothing else
here was edited or regenerated by the 2026-08-09 docs restructure that
relocated these files out of `docs/` root; only the reading/writing code's
path literals changed then.
...
fixture" describe block. Originally captured 2026-08-12 by
...
0.75s for production). **Regenerated 2026-08-16 (R-Q, `docs/work-in-
progress.md` §11 item 6) against the real jonatasgrosman/wav2vec2-large-xlsr-53-
{english,spanish} models via `scripts/measure-forced-alignment-hf.py`**,
superseding the original MMS-FA capture — MMS-FA (CC-BY-NC-4.0) is barred
from shipping by Decision 3, while production ships jonatasgrosman
(Apache-2.0). Same measurement convention (pad-sec 3.0, neighbour-midpoint
clamp) as the original capture, model backend only. Going forward, the CSV
and JSON of each pair are each other's baseline (the CSV must always match
the JSON it was derived from — enforced by the R-H describe block's own
per-word equality assertions). **A fidelity reference for the Rust
jonatasgrosman/ONNX/Viterbi port, not a claim about what Apply Sync should
commit** — full caveat text is embedded in both files' own headers, not just
here. Distinct from `phase4-fa-second-baseline-*` below (R-H's *second
baseline*, also added 2026-08-16) — that pair measures the real PRODUCTION
chunked pipeline's segment-level timing output; this pair measures per-word
Viterbi/windowing fidelity under the historical per-segment pad-sec-3.0
convention. Both address R-H; only this one was ever in scope for R-Q.

| File pattern | Purpose |
|---|---|
| `phase4-fa-tokens-{v6,173,spanish}.json` | Raw per-word jonatasgrosman capture (`text`/`start`/`end`/`score`/`seg`), plus `_caveat` and `_provenance` (model, torch/torchaudio versions, pad-sec, language, token/segment counts, failed/dropped words). |
...
`headExtendFirstSegment`) `App.tsx`'s own FA branch runs. Column format is
byte-identical to `phase4-baseline-*-segments.csv`/`-skipped.csv` above, so
the two are directly diffable by segment `tag` (kept-segment order differs
between the two runs — FA achieved 0 skipped segments on all three projects,
Whisper's 3/3/1 skips gone — so `order` alone is not a valid join key).

| File pattern | Purpose |
|---|---|
| `phase4-fa-second-baseline-{v6,173,spanish}-segments.csv` | Real production-pipeline FA output — order/tag/text/startTime/duration/endTime/anchorSource (`anchorSource` always `forced-alignment`), one row per FA-kept segment. |
| `phase4-fa-second-baseline-{v6,173,spanish}-skipped.csv` | The skip set FA's run produced (empty header-only on all three projects this capture — 0 skips). |
...
### `fa-e2e-alignment-*.json` — full end-to-end alignment parity references (WS1 Task 5, Slice D4)

Generated by `scripts/capture-fa-e2e-reference.py` (which shells out to
`npx tsx scripts/generate-fa-e2e-tokens.ts`), over the same three real audio
windows as the `fa-onnx-emission-*.json` fixtures above — but carrying the
full pipeline's reference OUTPUT (per-token merged spans), not just an
emission matrix. `input_samples`/`emission_log_probs`-equivalent audio is
reused verbatim from the matching `fa-onnx-emission-*.json` fixture (D2);
target token ids come from driving the live, D3-proven
`src/services/faTextNormalize.ts` normalizer and mapping to vocab ids;
`expected_spans` come from REAL torchaudio 2.2.2
`forced_align`/`merge_tokens` — an independent C++ implementation of the same
DP `src-tauri/src/fa_viterbi.rs` was hand-ported from, not derived from any
Rust output. Read by `src-tauri/src/fa_onnx.rs`'s `e2e_parity` tests
(feature-gated, `fa-inference`) to assert the complete Rust path (model load
-> forward pass -> `fa::text` tokenize -> Viterbi -> merged spans)
reproduces this reference — hard gate: token span start/end frames
IDENTICAL, zero tolerance; score drift reported as an observation only. Does
not overwrite or read from `fa-emission-*.json` (MMS_FA) or modify
`fa-onnx-emission-*.json` (D2) in any way.

| File pattern | Purpose |
|---|---|
| `fa-e2e-alignment-en-deep-night.json` | "It is deep in the night." (en), 23 target tokens / 23 merged spans. |
| `fa-e2e-alignment-en-mother-look.json` | "Your mother does not look up." (en), 28 target tokens / 28 merged spans. |
| `fa-e2e-alignment-es-resultan-inutiles.json` | "pero resultan inútiles." (es), 22 target tokens / 22 merged spans. |
