...
// `docs/archive/history/history-2.md`'s Session AH entry) is: text filed into a window's silent
...
describe.skipIf(!MEASURE)('WS1 Session AH Step 3e — pre-registered predictions', () => {
  it('states, per row, whether S2 places a chunk edge at that seam', async () => {
    ...
    L.push('- **No silence.** `docs/archive/history/history-2.md`\'s Session AH entry records no detected silence within');
