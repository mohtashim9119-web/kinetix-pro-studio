...
- **A test that asserts only final settled DOM state can pass with the bug reintroduced — mid-flight state must be held pending and inspected.** A green end-state assertion is compatible with a regression that only corrupts the in-progress UI (sibling rows flipping to "Checking…" while one pack downloads, WS2 T4.7). Hold the row in its mid-flight state and assert there, not only after the download settles.
- **A digest or other expensive derived value computed during a write must be returned and reused under the same name — never recomputed under a different name in a second read of the same bytes.** WS2 T4.8's class: a post-download verify hash and a sidecar write each re-read the just-finished file; on macOS that second read can race Spotlight indexing and hang indefinitely. The fix is to delete the redundant read and thread the verify digest through to the sidecar writer.
...
- **Never revert a probe mutation with `git checkout <file>` while uncommitted work exists in that file — commit first, or copy the file aside and restore by move.** `git checkout` restores the whole file from HEAD, not just the probe hunk; any real edits sitting in the same file are discarded with no undo. Worked instance: WS2 T4.8 lost committed edits twice this way when reverting a probe mutation.
...
| `git checkout <file>` to revert a probe while uncommitted edits exist in that file | Restores the whole file from HEAD and silently drops every uncommitted edit in it — commit first, or copy aside and restore by move |
...
**`docs/work-in-progress.md`'s structure contract (permanent).** Every workstream in that file (WS1, WS2, and any future one) carries exactly five sections, in this fixed order, with these exact names: (1) Finished but pending verification — complete, a proof step still outstanding; (2) In progress — required tasks listed first, one **END GOAL:** line at the bottom (mandatory even when the section is `(none)`); (3) Next tasks; (4) Open bugs; (5) Deferred tasks. Sections are never renamed, reordered, or omitted — a new workstream is created with all five headings present even if empty. The file also ends with a file-level **Backlog Items** section — cross-workstream defects and follow-ups tagged `[OPEN · NON-BLOCKING]`, one line each (≤30 words). A workstream carries no dedicated "finished work" section in this file at all — a closed item is dropped from whichever section it occupied and folded into the workstream's own top-of-section `Status:` line (one line, prose) plus its full record in `docs/history-2.md`; this file never grows a running list of completed pointers. The short contract summary at the top of `docs/work-in-progress.md` itself is this entry's mirror, not a second source of truth; this entry is authoritative.
