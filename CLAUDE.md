...
- **Active workstream folders** → `docs/ws1-sync-pipeline/` (sync pipeline), `docs/ws3-video-segments/` (video-segment drag/preview defects). A folder is deleted once its workstream closes, with its content folded into `docs/history.md` — `docs/checklists/` is the deliberate exception (a standing, repeated procedure, not a one-time investigation).
- **Standing manual verification procedure** → `docs/checklists/wkwebview-drag-checklist.md` — run before any release and after any change to `dragSession.ts`/`dragCascade.ts`/`dragGeometry.ts`/timeline CSS.
- **Ruling records** → `docs/decisions/*.md`, one file per ruling (Model P, last-segment-edge lock, pointercancel discard-vs-commit, undo/redo design). `project-state.md`'s "Rulings In Force" section indexes them.
- **ffmpeg sidecar re-provisioning** → `src-tauri/binaries/README.md`.

**CSV/measurement files in `docs/` root are permanent — do not move them.** `docs/measurements/` holds *scored/superseded* measurement data. Anything still sitting in `docs/` root (the `phase4-baseline-*.csv` golden baseline, `verification-baseline.csv`, the `*-Smear-Phase2a.csv` exports, `v6-smear-baseline.csv`, `173-smear-baseline.csv`, the core `phase3-onset-*-fa*.csv` tables, and others) is there because a `scripts/*.py` or `scripts/*.test.ts` file reads it by *hardcoded path*. An attempted move of these into `docs/measurements/` on 2026-08-09 broke 5 tests immediately, including the golden-replay test (`phase4-handoff-replay-sync.test.ts`) — caught before commit, reverted the same session. Before moving any file out of `docs/` root, grep `scripts/` for its filename; if there's a hit in a `.py`/`.ts` file (not a `.md` runbook or docstring), it stays. See `docs/measurements/README.md` for the full accounting of what was already safely moved.
