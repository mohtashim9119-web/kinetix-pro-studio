...
- **Active workstream folders** → `docs/ws1-sync-pipeline/` (sync pipeline), `docs/ws3-video-segments/` (video-segment drag/preview defects). A folder is deleted once its workstream closes, with its content folded into `docs/history.md` — `docs/wkwebview-drag-checklist.md` is the deliberate exception (a standing, repeated procedure, not a one-time investigation).
