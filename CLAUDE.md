...
| ReviewMappingModal thumbnail is a static image, not a live overlay render | Editing text/font/color/position in the modal doesn't update the thumbnail preview — user must close the modal to see the result | Extract a shared overlay-render component (font+position scaled by container ratio) usable by both `PreviewStage` and the thumbnail; live-update on segment state |
| Heading text has no italic support | `HeadingConfig` has no `fontStyle` field; scene overlay text supports italic end-to-end, heading does not. No UI control exists for it (deliberately — would be dead without the renderer wiring below) | Add `fontStyle` to `HeadingConfig` (`types.ts`) + apply in `PreviewStage`'s heading `<h1>` + `frameRenderer.ts`'s heading canvas font string + add the modal toggle |
| Export ignores fontWeight/fontStyle/textShadow on the main overlay caption | `frameRenderer.ts:475-477` computes `fontWeight`/`fontStyle`/`shadow` from `overlayConfig` but the canvas font string at line 488 is hardcoded to `` `italic normal ${bodyPx}px "${fontFamily}"` `` — exported captions are always italic+normal-weight regardless of user settings, and `shadow` is never applied at all (dead read). `PreviewStage`'s `<p>` for the same caption *does* apply these dynamically, so preview/modal show the right look but the exported MP4 won't match. Not manually tested this session | Wire `fontWeight`/`fontStyle` into the canvas font string and call `applyTextShadow(ctx, shadow)` (existing helper, already used for extra overlays at line 278), mirroring `PreviewStage` |
