...
## Amendment 3 (2026-09-17, FA wiring audit @ `ebec58a`)

**Current ruling:** Local forced alignment is wired and ships in **CI-built** desktop installers (`npm run tauri:build -- -f fa-inference`, `.github/workflows/build.yml:424`). Cloud FA is optional acceleration, not first delivery of the capability. The project toggle still defaults off at read time (`FA_PROJECT_DEFAULT_ON = false`, `faGate.ts:91`) until implementation dispatch under NR-2.

> **Prior claim (2026-09-16 investigation @ `c463814`, superseded):** “Cloud forced alignment is not a migration of a live feature… `fa-inference` is off in every shipped Cargo build… every installer produced to date returns `notImplemented` from `fa_align`. Word-level precision sync has never reached a user.” That was accurate for plain `npm run tauri:build` without `-f` and for the toggle default; it was **incorrect** for GitHub Actions installer artifacts, which compile `fa-inference` since `build.yml:416-424`.

Cloud FA still reduces on-disk pack downloads (1,262,512,711 to 1,262,619,311 bytes per language; 6,312,776,755 bytes for all five) for users who prefer cloud alignment over local packs.

---

Cloud forced alignment is not a migration of a live feature. ~~`FA_PROJECT_DEFAULT_ON` is false (`faGate.ts:91`) and `fa-inference` is off in every shipped Cargo build (`Cargo.toml:110`), so every installer produced to date returns `notImplemented` from `fa_align`. Word-level precision sync has never reached a user.~~ **(Struck — see Amendment 3.)** The product consequence of this plan is that cloud FA becomes available without requiring users to download 1,262,512,711 to 1,262,619,311 bytes per language (6,312,776,755 bytes for all five packs).
