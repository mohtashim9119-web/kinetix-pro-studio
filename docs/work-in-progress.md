...
## Preview stall — fix implemented, awaiting owner manual test
Opened: 2026-08-10 | Status: cause confirmed, fix in tree, NOT verified in the real app

- [ ] Video segment preview stalls/blacks-out/drops transitions when a long clip is slipped toward its end (Case B, near max slip). Repro: ~10s clip in a ~3.4s segment, slip toward the end. Preview only — exports render correctly from the same `trimStart`. Reproduces on a freshly reloaded project sitting at max slip with **no drag gesture involved**, which clears every gesture/timing-race theory as the cause. Five fix attempts across the trim math, the asset model, and the decode pool's concurrency/range-lookup logic all landed real, kept improvements but none resolved this. **Update 2026-08-10 (H1 CONFIRMED):** the legacy-`<video>` A/B toggle isolated the cause to the WebCodecs decode pool; Part A `ffprobe` on the actual failing clip found **1 keyframe in the entire 10s/240-frame clip** — confirms H1 in its strongest form. Mechanism, re-verified against current `main`: `MAX_BUFFERED_FRAMES_PER_SESSION=90` at 24fps is a 3.75s budget from the single keyframe at t=0; `handleDecoderOutput` drops (not evicts) overflow frames once the cap is hit, so a deep target's own frame is decoded then discarded, and the session freezes on the last frame that made it into the buffer, entrenched by `retainedFloorUs` advancing to that stale timestamp. Export is unaffected — it uses a separate sequential-decode generator with real backpressure, never a capped drop. **Fix implemented 2026-08-10 (sliding window), not yet manually verified.** An owner library scan found 6 of 7 clips carry exactly one keyframe, making this the default path, not an edge case — which withdrew the audit's earlier legacy-`<video>` fallback recommendation (it would strip GL transitions/zoom/grading from nearly every video segment). `handleDecoderOutput` now evicts frames the target has already passed instead of dropping newly-decoded ones, holding a window of `RETAIN_BEHIND_SEC` (0.5s, new) behind + `WINDOW_AHEAD_SEC` (1.5s) ahead; reachable depth becomes unbounded, memory ceiling unchanged. `retainedFloorUs` re-specified from "last selected frame" (a ratchet that entrenched the stall) to "oldest frame still buffered"; `MAX_SESSION_FRAMES` removed as the same defect at ~25s depth. Confined to `videoDecoderPool.ts` — no consumer changed, GL layer untouched. `WS3_MEASURE` instrumentation removed; `FORCE_LEGACY_VIDEO_PREVIEW` kept at `false`. 7 new tests on a real-shaped single-keyframe fixture, verified red before / green after by reverting the source. **Owner manual test is the gate — five prior passes also had passing tests and did not fix the symptom.** Full account: `docs/ws3-video-segments/ws3-audit.md`'s "H1 CONFIRMED" and "Sliding-window fix" sections.
