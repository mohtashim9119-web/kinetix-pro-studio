...
- **Verification:** manually verified by user 2026-07-29 — T1 parentheses-sync (timeline unchanged/improved, passed) and T2 294-segment regression (294/294 matched, no new skips/aborts, passed). Middle-gap and special-chars fixtures: automated only (no manual run performed for WS4 specifically).

**WS5 — S3 verification + speaker-label stripping (13a extension) + R8 threshold lock: COMPLETE, manually verified 2026-07-29 (T1/T2). R5/N4 parser fix: INVESTIGATED, deliberately NOT fixed.**
...
- **Verification:** **manually verified by user 2026-07-29** — T1 294-segment project (294/294 matched, no new skips/aborts, passed) and T2 speaker-label scene doc (labels stripped, sync unchanged/improved, passed). Middle-gap fixture: automated only (no manual run performed for WS5 specifically). The 294/294 and 8-of-10 figures cited in `syncConstants.ts`'s justifications are carried forward from the WS1b/WS-logs passes that measured them, not re-measured here.
