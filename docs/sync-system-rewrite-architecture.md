...
| Fixture tuning pass | R8's threshold derivation on the four named fixtures; final constants recorded in `syncConstants.ts`. May also explore stemming/fuzzy matching (extending the existing near-match prefix credit, S3) if the fixture data shows a need — not a separate deferred item. |
