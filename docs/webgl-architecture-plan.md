...
**Status:** Audit (Section 1) and engine decision + feasibility spike (Sections 2–3) are **COMPLETE** — produced as part of writing this document, 2026-07-10. **All build phases (Section 6) are NOT STARTED.** The verdict: rebuild the effects-rendering engine (transitions, animations, and the new color-grading feature area) on **WebGL2**, not WebGPU — GO confirmed empirically on both Chromium and this project's real Tauri WKWebView, including the single most important integration question (WebCodecs `VideoFrame` → GPU texture upload), which passed 312/312 frames with zero GL errors on both engines.
...
- **Phase 1 — Core compositor module** (Section 6). The `VideoFrame`-native WebGL2 compositor, context-loss handling, image-segment texture path, pure/testable parameter derivation.
