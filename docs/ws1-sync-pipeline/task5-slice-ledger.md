...
| **D25** (D17-D24 are recorded in their own dated docs + the Known-gap register below, not as `§1` rows — this row is added per this slice's own explicit task instruction) | **A1**: wired D24's unwired `ensure_durable_wav` into `fa_align_dev` (the one real caller, dev-only/console-invoked, unchanged scope from D10) and exercised it live against a REAL `AppHandle<Wry>` (via `tauri::test::mock_context::<Wry,_>` with an empty window list — avoids needing a running app/webview — and a new `harness = false` integration-test binary, `src-tauri/tests/fa_durable_wav_live.rs`, since the real Wry runtime's `EventLoop::new()` needs the process's actual main thread, which libtest's own worker-thread model can't provide) and the real 173 corpus audio. **Caught and fixed a real bug live on the first run**: the `.tmp` file's name ended in `.tmp`, not `.wav`, so ffmpeg's own filename-based output-format auto-detection failed on every real transcode — untested until this slice exercised it with a real ffmpeg invocation. Fix: tmp files now live under `cache_dir/.tmp/{key}.{uuid}.wav`. Proved reuse three ways (byte-identical content, mtime re-stamped past an artificial aging, and — the real distinguishing signal — 1538x wall-clock speedup on a hit vs. a miss, isolated from D10's own unrelated ~74-78s model-manifest-SHA-256 fixed cost). **A2**: found and fixed a second real bug (concurrent misses on the same source shared a deterministic tmp filename — two real ffmpeg processes would race to write the same file; fixed with a per-call UUID suffix); confirmed the `.tmp` write location is the destination directory (required for `rename` atomicity); confirmed eviction fires at the REAL 2 GiB cap and protects a recently-touched entry over creation order; confirmed an edited source mints a new key end-to-end and never serves the old entry. **A3**: re-confirmed 118 chunks under index attribution live, and derived (from `faChunkPlan.ts`'s own doc comment: 149 total runs, 31 zero-duration) why the count differs from time attribution's 97 — `149 − 31 = 118` audio-bearing runs, all of which receive text under index attribution's exhaustive per-token join, vs. only 97 of them happening to contain a segment's own stale `startTime` under time attribution's coarser join. **B1** (read-only): established that R.5's trigger condition (real unscripted audio between two segments sharing a chunk) is still fully reachable under the current index-attribution chunked path — D21's fix addressed a DIFFERENT, coarser-granularity bug (which whole segment's text lands in which window), not the within-window content gap R.5 exists to close; confirmed no wildcard/star-token mechanism exists anywhere in `fa_viterbi.rs`/`fa_onnx.rs` (grepped), and that segment boundaries don't survive `faChunkPlan.ts`'s own text-flattening (`.join(' ')`) into the text handed to the Rust decode step, under either attribution mode — verdict **(i) still reachable**, no implementation. Full detail: `docs/ws1-sync-pipeline/d25-durable-cache-live-wired-r5-scoping-2026-08-14.md`. | (this slice's own commit) | `cargo check` clean both configs; `cargo test --lib` 72→**76** (+4, A2); `cargo test --lib --features fa-inference` 146→**150 passed, 0 failed, 19 ignored** (+4, ignored count unchanged); `cargo clippy --all-targets --features fa-inference` same 4 pre-existing warnings, 0 new (one incidental private-type-leak warning surfaced by a `pub mod` visibility widening was fixed in the same pass, not left as a 5th); `npm run lint` clean; `npm test` 79 files/1942 passed/1 skipped, unchanged; golden replay 6/6; live zero-fallback regression guard re-run: `wall_clock=74.4s chunks=118 words=1643 fallback=0`, holds. |
...
scope, so before building it, D25 asked whether the index-attribution fix
(D21) already eliminated the condition R.5 exists to handle. **It did not —
R.5's trigger condition (real unscripted audio between two segments sharing
a chunk) is still fully reachable today, verdict (i).** D21 fixed a
DIFFERENT, coarser-granularity bug (whole-segment text landing in the wrong
window); it did not add any wildcard/gap-absorption mechanism, and segment
boundaries still do not survive `faChunkPlan.ts`'s own text-flattening into
the text handed to the Rust CTC decode, under either attribution mode — 172
segments across 118 real chunks (D25 A3's own live re-run) means most
chunks already concatenate multiple segments' text today. Full evidence and
the smallest-implementation sketch (two layers: `faChunkPlan.ts` would need
to preserve per-segment text spans per chunk instead of one joined string,
and `fa_viterbi.rs`'s Viterbi DP would need a genuine zero-cost wildcard
state, not the standard CTC blank it has today):
`docs/ws1-sync-pipeline/d25-durable-cache-live-wired-r5-scoping-2026-08-14.md`.
