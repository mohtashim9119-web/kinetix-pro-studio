...
| `fa-vocab-representability-2026-08-12.md` | WS1 corpus representability measurement | 2026-08-12 | `faTextNormalize.ts` run against real project script text (en/es) and Common Voice fallback sentences (fr/de/pt, no real project text in this corpus) for all five shipping vocabs — unrepresentable-word rate, reason breakdown, offending-character census, and the digit-deferral verdict for Phase 3b. Measurement only, `faTextNormalize.ts` untouched. |
| `runtime-unblock-2026-08-12.md` | Runtime unblock investigation (Task 5 pre-work) | 2026-08-12 | Follow-up to `runtime-spike-2026-08-11.md`'s G4: the `ort`/onnxruntime ≥1.27 deadlock is resolved — `ort`'s `api-NN` Cargo features are a configurable version floor (default 27, base 17), not a hard requirement; disabling them lets the existing onnxruntime-osx-x86_64-1.22.0 binary load and run a full forward pass through the real Rust `ort` binding (fidelity-verified against the torch reference, 0/49 argmax mismatches). Also corrects the last-x86_64-macOS-release finding from v1.22.0 to v1.23.2. No from-source onnxruntime build needed; `src-tauri/Cargo.toml` untouched. |
| `d10-runtime-observations-2026-08-13.md` | WS1 Task 5 Slice D10 (`3787b11`) live-run findings | 2026-08-13 | Whole-file memory duration ladder (30/60/120/240s → super-linear peak-memory growth, extrapolating to 60-150GB at production length vs. 32GB available — the finding that makes windowing mandatory); FA-vs-Whisper `anchorStart` delta over 64 real segments; `exp(score)` confidence range across all 6 e2e fixtures ([0.730, 1.0], making R.7's `CONF_MIN` gate untestable today); `conv_stride`→seconds-per-frame sourcing. Per-item provenance noted inline — some figures corroborated by the D10 commit message, others recorded as operator-reported since the dev harness's console output isn't captured to a committed file. Feeds the rulings in `docs/ws1-sync-pipeline/task5-slice-ledger.md` §4. |
| `d11-chunked-alignment-2026-08-13.md` | WS1 Task 5 Slice D11 (`eda13b1`) real-corpus measurements, produced at Slice D12's (`dda07b7`) landing pass | 2026-08-13 | 240s excerpt whole-file-vs-windowed agreement table, per-chunk word-density breakdown, 709s full-project memory/wall-clock, the 2/97 CTC-infeasibility cases with input-shape derivation, and an explicit "conclusions NOT yet supported" section — later provenance-repaired and annotated with which conclusions D12 superseded by Slice D13, see the next row. (Indexed here retroactively, Slice D13 — this row itself did not exist before Slice D13's provenance-repair pass.) |
| `d13-index-attribution-2026-08-13.md` | WS1 Task 5 Slice D13 real-corpus measurements | 2026-08-13 | Whether an index-derived text-attribution rule (`faChunkPlan.ts`'s `computeFaChunkPlanWithAttribution`) reaches D12's oracle (`attribution_isolation`) bound without needing the oracle. B-control (disentangles D12's oracle boundaries from its oracle text); the combined ladder/B/B-control/index-7s/index-45s agreement table; Whisper-ground-truth triage showing index-45s statistically indistinguishable from the whole-file reference itself; a real coalescing bug this slice's own real-corpus run caught and fixed before any number here was accepted; and a derived (not fitted) agreement budget proposal. |
