...
- **ONNX-export step for the five jonatasgrosman models is scoped nowhere.** Still true — no task and no design-doc section covers converting/exporting the per-language PyTorch checkpoints for the native, bundled-in-binary runtime. What the ONNX-export question was itself an unscoped input *to* is now settled: ruling **R-L** (`project-state.md` §5, 2026-08-11) closes the runtime-delivery decision — out-of-process delivery (an external alignment process, Python or otherwise; call it the runtime-delivery "Option A," a distinct decision from Decision 8's heading-wildcard Option A, §4 Step V above) is rejected permanently, native in-process inference compiled into the binary is the permanent target, and `ffmpeg`/`whisper-cli` remain sidecars unaffected (R-L governs alignment only). The go-forward sequencing is **D-then-C**: land the capability-gated boundary first (Task 5's Stage 1 timing-interface slot, gated off until ready) before implementing native inference behind it — not the reverse. The ONNX-export sizing itself (this bullet's original subject) is still open and still Task 5's own scoping pass to close.
