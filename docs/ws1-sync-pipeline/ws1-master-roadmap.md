...
## NEXT UP (last updated 2026-08-13, after Task 5 Slices D1-D10 — STALE again as of D25, see note)

> **This block predates Task 5's actual execution and is kept below only as
> the historical scoping record it was written as.** As of 2026-08-14, Task
> 5's Rust integration has shipped through **Slice D25** (D1-D6, D8-D25; D7
> was cancelled as scoped, no commit exists under that name — full
> cancellation reasoning at `task5-slice-ledger.md` §2) behind the
> OFF-by-default `fa-inference` Cargo feature, reachable in the running app
> only via the DEV-only `window.__faDevAlign` path. Since this block was
> written, real chunked-windowing execution landed on top of R.0/R.1/R.4
> (D11), index attribution became the chunked path's own internal default
> and closed the real CTC-infeasibility cases it was built to fix
> (D13/D21/D22), R.2 padding was built, measured net-unfavorable, and
> deleted (D23/D24), a capability-gated Settings toggle and a word-timing
> schema shipped but remain inert (D17-D19), and a durable audio cache was
> built and live-wired against a real `AppHandle` (D24/D25) — still no
> production (UI-reachable) caller. Full slice-by-slice record, corrected
> assumptions, and rulings: `docs/ws1-sync-pipeline/task5-slice-ledger.md`.
> Current per-item status board (every sub-item done/in-progress/blocked,
> with evidence): `docs/ws1-sync-pipeline/task5-status-board.md`. **What
> genuinely remains next** is the capability-gated production wiring slice
> (a production, non-dev `AppHandle`-reachable caller of `fa_align`) — R.2 is
> now closed-negative (not remaining work), R.5 is scoped-but-unbuilt
> (verdict (i), still reachable — `task5-open-decisions.md` §2 for the
> whether/when call), and R.3/R.7-R.9 remain design-only. See the status
> board and `task5-slice-ledger.md`'s "Known-gap register" (§6), which
> supersedes the "Files expected to change" / "Acceptance criteria" text
...
- `src-tauri/Cargo.toml` — new ONNX Runtime dependency; `src-tauri/binaries/README.md` — model
  ...
  required before release, not before Task 5 (ruling R-D, `project-state.md` §5 — itself resting
  on ruling R-K, no release build until WS1 completes; if release timing changes, R-D must be
  revisited before Task 5 ships).
...
| Phases in progress, dev-only | 3 (forced alignment — Slices D1-D6/D8-D25 shipped (D7 cancelled as scoped) behind `fa-inference`, dev-only reachable; capability-gated production wiring is the one remaining engineering slice — R.2 closed-negative, R.5 scoped-but-unbuilt (verdict (i)), R.3/R.7-R.9 design-only — `docs/ws1-sync-pipeline/task5-status-board.md` is the current per-item board, more current than this table's 2026-08-11 build date) |
...
| CI | None for the four project-wide gates (`lint`, `npm test`, golden replay, `cargo check`) — still run by hand, see §9. One narrow exception since 2026-08-13: `.github/workflows/fa-ort-matrix.yml` (Slice D14 B1) automates the FA `fa-inference`/ORT four-way skip/hard-fail matrix on push/PR touching the FA Rust surface. |
...
| **3** | Stage 1 | **Forced alignment — the real timing-source upgrade.** This is the phase that actually fixes the smear defect. | **IN PROGRESS, dev-only.** Real ONNX+Viterbi Rust alignment, chunked windowing execution, index attribution, a capability gate, and a durable audio cache landed (Slices D1-D6, D8-D25; D7 cancelled as scoped — `docs/ws1-sync-pipeline/task5-slice-ledger.md`), behind the OFF-by-default `fa-inference` feature, reachable in the app only via the DEV-only `window.__faDevAlign`. Not yet the production timing source: the capability-gated production-wiring slice (a non-dev `AppHandle`-reachable `fa_align` caller) is the one remaining engineering item — R.2 is closed-negative (built, measured net-unfavorable, deleted), R.5 is scoped reachable-but-unbuilt (verdict (i)), R.3/R.7-R.9 remain design-only. Current per-item status: `docs/ws1-sync-pipeline/task5-status-board.md`. | `docs/ws1-sync-pipeline/task5-slice-ledger.md`; `docs/ws1-sync-pipeline/task5-status-board.md`; `roadmap-2026-08-07.md` §D2; `sync-pipeline-v2-plan.md` Steps M-Z |
...
- **No automated CI exists for the four project-wide gates.** `.github/workflows/build.yml` is `workflow_dispatch`-only and cross-builds installers; it contains no `npm test`/`vitest`/`cargo test`/`tsc` step. `npm run lint`, `npm test`, the golden-replay test, and `cargo check` are still run by hand, by whoever is making the change — there is no safety net catching a regression in these that isn't run for. **Narrower exception since 2026-08-13 (WS1 Task 5 Slice D14 B1):** `.github/workflows/fa-ort-matrix.yml` runs on push/PR touching the FA Rust surface and automates one specific thing — the `fa-inference`/ORT four-way skip-vs-hard-fail matrix (feature-off, feature-on×ORT-unset, feature-on×ORT-set, feature-on×`FA_REQUIRE_ORT`-with-no-dylib) — not the four gates above.
- **Permanent process rule (added 2026-08-12, ort-runtime-unblock investigation):** during WS1 implementation work, only `docs/work-in-progress.md` and files under `docs/ws1-sync-pipeline/` are written mid-task. `CLAUDE.md`, `docs/history.md`, and `project-state.md` are batched to task completion and require explicit owner approval before any edit — never touched at the end of a part or sub-task without it. R-M's superseded status and R-N's now-answerable packaging question (both touched by `docs/ws1-sync-pipeline/measurements/runtime-unblock-2026-08-12.md`) are formal `project-state.md` edits under this rule — **deferred to the end-of-Task-5 documentation pass**, pending owner approval.
...
- **No automated test CI for the four project-wide gates** (§9) — `lint`, `npm test`, golden replay, `cargo check` are still run by hand; accepted, no CI pipeline planned for these. Narrower exception since 2026-08-13: `.github/workflows/fa-ort-matrix.yml` automates the FA `fa-inference`/ORT four-way matrix specifically (task5-slice-ledger.md §6).
...
- **ONNX-export step for the five jonatasgrosman models — CLOSED 2026-08-11 (runtime spike, G4).** The export itself is proven viable: English exported cleanly (24.98s, 1.176 GiB, fidelity-verified against torch — max_abs_diff 0.000269, argmax path identical, see G5 in the runtime-spike measurement file). What remains genuinely unscoped is different from what this bullet used to name: **native-library provisioning for macOS x86_64.** The `ort` Rust crate requires onnxruntime ≥1.27, and no onnxruntime distribution (GitHub or PyPI) exists for macOS x86_64 at any version ≥1.24 — this project's own CI builds a universal x86_64+arm64 binary via `lipo`, so the default `ort` path cannot even build for one of its two target architectures. Only remaining path is a from-source onnxruntime build for x86_64-apple-darwin (ruling R-M, `project-state.md` §5), same category of cost as the existing whisper-cli from-source CI pattern — not yet scoped as its own CI work item. Separately, ruling **R-L** (2026-08-11) already closed the runtime-DELIVERY decision — out-of-process delivery (an external alignment process, Python or otherwise; call it the runtime-delivery "Option A," a distinct decision from Decision 8's heading-wildcard Option A, §4 Step V above) is rejected permanently, native in-process inference compiled into the binary is the permanent target, and `ffmpeg`/`whisper-cli` remain sidecars unaffected (R-L governs alignment only) — but which of the two in-process PACKAGING readings (static-link vs. load-dynamic+bundled-dylib) applies is still open (ruling R-N, deferred). The go-forward sequencing is **D-then-C**: land the capability-gated boundary first (Task 5's Stage 1 timing-interface slot, gated off until ready) before implementing native inference behind it — not the reverse. **Update, 2026-08-12:** the export procedure itself is no longer a one-off English-only spike script — it's now `scripts/export-fa-onnx.py`, a committed, repeatable, per-language export with a torch-vs-onnxruntime argmax-equivalence verification gate (fails loudly rather than emitting an unverified model), run and passing for all five languages (0 argmax mismatches each), with provenance recorded at `scripts/fixtures/fa-onnx-manifest.json` (hashes only, no weights committed). This closes the "ONNX export scoped nowhere" gap — the native-library-provisioning blocker (`ort`/onnxruntime for macOS x86_64) described above is unaffected and still open.
