...
|---|---|---|
| U25 | `ExportErrorKind` may gain or be renamed by item F | `EXPORT_FAILURE_COPY` is a **total record** (`satisfies` + `[K in ExportErrorKind]`). Adding/renaming a kind without a matching copy row is a `tsc` error. Verified still total after rebase onto `5a96f92` (item F mapped `ExportFailureVia` onto the same 11 kinds; no copy rows added). Do not invent names. |
