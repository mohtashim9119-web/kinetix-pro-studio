...
| Current HEAD | `2516a7c` on `main`, fully pushed to `origin/main`. Clean-slate sync rebuild complete (3a–3d-2); dead sync-wizard UI removed (`4890ea6`) — `handleApplySyncFromFiles` is the single live sync entry point; Step 5 complete (5.1–5.4) — headings now live only in the segments array, dual storage gone. |
...
**Step 7 — final regression:** ⬜ Not started. The only remaining Architecture Shift work — a full regression pass covering the clean-slate re-sync rebuild (3a–3d-2) together with Step 5 (headings array-only, 5.1–5.4) end-to-end.

**Restore tag:** sync-known-good-2026-06-23 (commit a1a326d).
**Newer reference point:** commit `2516a7c` — current `main` HEAD, post 3a+3b+3c+3d-1+3d-2 (clean-slate sync rebuild) + dead sync-wizard cleanup (`4890ea6`) + Step 5 5.1–5.4 (headings array-only, dual storage gone), pushed to `origin/main`.
---
