 ...
 * the real `src/services/webcodecsExport/exportWorker.ts` (production,
 * UNTOUCHED) with exactly ONE change: `HARDWARE_LADDER` below is forced to
 * `['prefer-software']` only, so this worker's `VideoEncoder` never selects
 * a hardware (VideoToolbox) encoder. Exists solely so
 * `docs/webcodecs-export-plan.md` Step 8's quality comparison can produce a
 * genuine software-encoded export to compare against the real hardware path
 * (the unmodified `exportWorker.ts`, driven by the real
 * `exportPipelineWebCodecs.ts`) and the legacy libx264 path — per Step 8's
 * own instructions: "do NOT modify exportWorker.ts ... OR modify the spike
 * to construct its own worker with the software config." This file, plus
 * its sibling `exportPipelineWebCodecsSoftwareSpike.ts` (which points at
 * THIS worker instead of the real one), is that "construct its own worker"
 * approach. Not imported by any production file. Delete alongside the rest
 * of this spike directory once Step 8 is reviewed.
 *
 * Only two diffs from the real file: (1) import paths adjusted for this
 * file's different directory depth (src/dev/webcodecsStep2Spike/ vs.
 * src/services/webcodecsExport/), (2) `HARDWARE_LADDER` below. Everything
 * else — including this header's own original text below — is verbatim.
 *
...
// STEP 8 SPIKE OVERRIDE — forced software-only (see this file's header). The
// real exportWorker.ts has: ['prefer-hardware', 'no-preference', 'prefer-software'].
const HARDWARE_LADDER: HardwareAcceleration[] = ['prefer-software'];
